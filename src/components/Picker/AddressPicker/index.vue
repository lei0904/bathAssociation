...
<script>
  ...
  export default{
    ...
    mounted () {
      this.addressProvince = this.defaultAddressProvince
      this.addressCity = this.defaultAddressCity

      let addressProvinceIndex = 0
      let addressCityIndex = 0

      if (this.addressProvince) {
        Object.keys(address).forEach((item, index) => {
          if (this.addressProvince === item) {
            addressProvinceIndex = index
            if (this.addressCity) {
              address[item].forEach((item2, index2) => {
                if (item2 === this.addressCity) {
                  addressCityIndex = index2
                }
              })
            }
          }
      ...
      this.$nextTick(() => {
        this.addressSlots[0].defaultIndex = addressProvinceIndex
        this.addressSlots[2].defaultIndex = addressCityIndex
      })
    },
    methods: {
